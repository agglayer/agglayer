            ...
            AgglayerProverType::CpuProver(cpu_prover_config) => {
                ...
                Self::build_local_service(
                    ...
                    LocalExecutor {
                        ...
                        proving_key,
                        verification_key,
                        timeout: cpu_prover_config.proving_timeout,
                    },
                )
            }
            AgglayerProverType::MockProver(mock_prover_config) => {
                ...
                Self::build_local_service(
                    ...
                    LocalExecutor {
                        ...
                        proving_key,
                        verification_key,
                        timeout: mock_prover_config.proving_timeout,
                    },
                )
            }
            agglayer_config::prover::AgglayerProverType::GpuProver(_) => todo!(),
        };
...
impl Service<Request> for Executor {
    ...
    fn call(&mut self, req: Request) -> Self::Future {
        ...
        let fut = async move {
            ...
            //     (Some(network), None) => ,
            //     (Some(network), Some(mut fallback)) => match network.await {
            //         Ok(res) => Ok(res),
            //         Err(err) => {
            //             error!("Network prover failed: {:?}", err);
            //             fallback.ready().await?.call(req).await
            //         }
            //     },
            //
            //     (None, Some(mut local)) =>
