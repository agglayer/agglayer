...
#[derive(Debug, Clone)]
pub struct InconsistentVariant;
...
impl TryInto<core::AggchainData> for PayloadWithCtx<Payload, Ctx> {
    type Error = InconsistentVariant;
        ...
        Ok(match (payload, ctx) {
            ...
            (Payload::MultisigOnly(payload), Ctx::MultisigOnly(ctx)) => {
                core::AggchainData::MultisigOnly(
                    PayloadWithCtx(payload, ctx)
                        .try_into()
                        .map_err(|_| InconsistentVariant)?,
                )
            }
            (
                Payload::AggchainProofOnly {
                    aggchain_proof: payload,
                    ..
                },
                Ctx::AggchainProofOnly(ctx),
            ) => core::AggchainData::AggchainProofOnly(
                PayloadWithCtx(payload, ctx)
                    .try_into()
                    .map_err(|_| InconsistentVariant)?,
            ),
            (
                ...
                multisig: PayloadWithCtx(multisig, multisig_ctx)
                    .try_into()
                    .map_err(|_| InconsistentVariant)?,
                aggchain_proof: PayloadWithCtx(aggchain_proof, aggchain_proof_ctx)
                    .try_into()
                    .map_err(|_| InconsistentVariant)?,
            },
            _ => return Err(InconsistentVariant),
        })
    }
}
