...
use crate::{
    aggchain_data::{
        CertificateAggchainData, CertificateAggchainDataCtx, CertificateAggchainDataWithCtx,
    },
...
impl LocalNetworkStateData {
    ...
    pub fn apply_certificate(
        ...
        let aggchain_data = {
            let certificate_aggchain_data: CertificateAggchainData =
                certificate.aggchain_data.clone().into();
            CertificateAggchainDataWithCtx(certificate_aggchain_data, aggchain_data_ctx)
                .try_into()
                .map_err(|_| Error::MissingAggchainVkey)? // todo proper error
        };
