...
use agglayer_settlement_service::{
    testutils::{
        mock_retrieve_revert, mock_retrieve_success, mock_settlement_revert,
        mock_settlement_success,
    },
    ClientError, ClientErrorType, MockSettlementServiceTrait, RetrievedSettlementResult,
    SettlementJobResult,
};
...
#[test_log::test(tokio::test)]
#[timeout(Duration::from_secs(2))]
async fn from_candidate_settle_fails_on_client_error() {
    let tmp = TempDBDir::new();
    let storage = new_storage(&tmp.path);
        ...
        .returning(|_| {
            Ok(RetrievedSettlementResult::Completed(
                SettlementJobResult::ClientError(ClientError {
                    ...
                    message: "simulated settlement failure".into(),
                }),
            ))
        });
...
#[test_log::test(tokio::test)]
#[timeout(Duration::from_secs(2))]
async fn from_candidate_settle_fails_on_not_found() {
    let tmp = TempDBDir::new();
    let storage = new_storage(&tmp.path);
    ...
    // Height should NOT advance — certificate goes to InError
    assert_eq!(next_expected_height, Height::ZERO);
...
#[test_log::test(tokio::test)]
#[timeout(Duration::from_secs(2))]
async fn from_candidate_settlement_revert_goes_to_error() {
    let tmp = TempDBDir::new();
    let storage = new_storage(&tmp.path);
