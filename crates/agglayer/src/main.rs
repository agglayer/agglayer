...
fn main() -> anyhow::Result<()> {
    ...
    match cli.cmd {
        ...
        cli::Commands::ProverConfig => println!(
            "{}",
            toml::to_string_pretty(&agglayer_prover_config::ProverConfig::default()).expect("Failed to serialize ProverConfig to TOML")
        ),
        cli::Commands::Config { base_dir } => println!(
            "{}",
            toml::to_string_pretty(&agglayer_config::Config::new(&base_dir)).expect("Failed to serialize Config to TOML")
        ),
        cli::Commands::ValidateConfig { path } => {
            match agglayer_config::Config::try_load(path.as_path()) {
                Ok(config) => {
                    println!("{}", toml::to_string_pretty(&config).expect("Failed to serialize ValidateConfig to TOML"));
                }
