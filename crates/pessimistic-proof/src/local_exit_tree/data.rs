...
mod tests {
    ...
    use rand::{random, rng as thread_rng, Rng};
    ...
    fn test_data_vs_frontier_root() {
        let num_leaves = thread_rng().random_range(1..100.min(1 << TREE_DEPTH));
        compare_let_data_let_frontier(num_leaves)
    }

    #[test]
    fn test_data_vs_frontier_add_leaf() -> Result<(), LocalExitTreeError> {
        let num_leaves = thread_rng().random_range(1usize..100.min(1 << TREE_DEPTH));
        let leaves = (0..num_leaves).map(|_| random()).collect::<Vec<_>>();
    ...
    fn test_merkle_proofs() {
        let num_leaves = thread_rng().random_range(1..=100.min(1 << TREE_DEPTH));
        let leaves = (0..num_leaves).map(|_| random()).collect::<Vec<_>>();
        let leaf_index = thread_rng().random_range(0..num_leaves);
