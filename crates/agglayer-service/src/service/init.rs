//! Agglayer service initialization
...
pub use super::AgglayerService;
use crate::{epoch_synchronizer::EpochSynchronizer, kernel::Kernel};

pub type L1RpcProvider = NonceManagerMiddleware<SignerMiddleware<Provider<Http>, ConfiguredSigner>>;

impl AgglayerService<L1RpcProvider, PendingStore, StateStore, DebugStore> {
    /// Create agglayer service from configuration.
    async fn from_config(
        ...
        let clock_ref = match &config.epoch {
            Epoch::BlockClock(cfg) => {
                ...
                let clock = BlockClock::new_with_ws(
                    alloy::providers::WsConnect::new(config.l1.ws_node_url.as_str()),
                    cfg.genesis_block,
                    cfg.epoch_duration,
        ...
        tracing::debug!("L1 RPC provider created");
        let rollup_manager = Arc::new(
            L1RpcClient::try_new(
                l1_rpc.clone(),
                PolygonRollupManager::new(config.l1.rollup_manager_contract, l1_rpc.clone()),
        ...
        // Construct the core.
        let core = Kernel::new(l1_rpc.clone(), config.clone());
        ...
        let (certificate_sender, data_receiver) = mpsc::channel(
            config
...
{
    /// Create agglayer service from individual components.
    pub fn from_parts(
        kernel: Kernel<Rpc>,
