...
// SAFETY: This struct has a stable C-compatible memory layout
unsafe impl Pod for ImportedBridgeExitZeroCopy {}
unsafe impl Zeroable for ImportedBridgeExitZeroCopy {}
...
impl ImportedBridgeExitZeroCopy {
    ...
    pub fn from_imported_bridge_exit(imported_bridge_exit: &unified_bridge::ImportedBridgeExit) -> Self {
        Self {
    ...
    pub fn to_imported_bridge_exit(&self) -> unified_bridge::ImportedBridgeExit {
        unified_bridge::ImportedBridgeExit {
            ...
            claim_data: unified_bridge::Claim::Mainnet(Box::new(unified_bridge::ClaimFromMainnet {
...
impl SmtMerkleProofZeroCopy {
    ...
    pub fn from_smt_merkle_proof(proof: &agglayer_tries::proof::SmtMerkleProof<agglayer_primitives::keccak::Keccak256Hasher, 192>) -> Self {
    ...
    pub fn to_smt_merkle_proof(&self) -> agglayer_tries::proof::SmtMerkleProof<agglayer_primitives::keccak::Keccak256Hasher, 192> {
        let siblings: [agglayer_primitives::Digest; 192] = self.siblings.map(|s| agglayer_primitives::Digest(s));
...
impl SmtNonInclusionProofZeroCopy {
    ...
    pub fn from_smt_non_inclusion_proof(proof: &agglayer_tries::proof::SmtNonInclusionProof<agglayer_primitives::keccak::Keccak256Hasher, 64>) -> Self {
    ...
    pub fn to_smt_non_inclusion_proof(&self) -> agglayer_tries::proof::SmtNonInclusionProof<agglayer_primitives::keccak::Keccak256Hasher, 64> {
        let siblings: Vec<agglayer_primitives::Digest> = self.siblings.iter().map(|s| agglayer_primitives::Digest(*s)).collect();
...
mod tests {
    ...
    fn test_zero_copy_with_non_empty_vectors() {
        ...
        let nullifier_path = crate::nullifier_tree::NullifierPath::<Keccak256Hasher> {
            siblings: vec![agglayer_primitives::Digest([0u8; 32]); 64],
                ...
                siblings: [agglayer_primitives::Digest([0u8; 32]); crate::local_balance_tree::LOCAL_BALANCE_TREE_DEPTH],
            };
        ...
        assert_eq!(reconstructed.balances_proofs.len(), 0); // Not reconstructed in zero-copy
    }
    ...
    fn test_bridge_exit_zero_copy_roundtrip() {
        ...
        assert_eq!(reconstructed.token_info.origin_network, bridge_exit.token_info.origin_network);
        assert_eq!(reconstructed.token_info.origin_token_address, bridge_exit.token_info.origin_token_address);
    ...
    fn test_token_info_zero_copy_roundtrip() {
        ...
        assert_eq!(reconstructed.origin_token_address, token_info.origin_token_address);
    }
    ...
    fn test_vector_operations_simulating_main() {
        ...
        let bridge_exits_zero_copy: Vec<BridgeExitZeroCopy> = vec![BridgeExitZeroCopy::from_bridge_exit(&bridge_exit)];
        let bridge_exits_bytes = bytemuck::cast_slice(&bridge_exits_zero_copy);
        
        println!("BridgeExitZeroCopy size: {}", std::mem::size_of::<BridgeExitZeroCopy>());
        println!("BridgeExitZeroCopy alignment: {}", std::mem::align_of::<BridgeExitZeroCopy>());
        println!("bridge_exits_bytes len: {}", bridge_exits_bytes.len());
        println!("bridge_exits_bytes ptr: {:p}", bridge_exits_bytes.as_ptr());
        println!("bridge_exits_bytes ptr alignment: {}", (bridge_exits_bytes.as_ptr() as usize) % std::mem::align_of::<BridgeExitZeroCopy>());
        
        // Simulate main.rs reading the vector data
        // Try direct cast_slice instead of try_cast_vec
        let reconstructed_bridge_exits = bytemuck::cast_slice::<u8, BridgeExitZeroCopy>(&bridge_exits_bytes);
        ...
        assert_eq!(reconstructed_bridge_exit.token_info.origin_network, bridge_exit.token_info.origin_network);
        assert_eq!(reconstructed_bridge_exit.dest_network, bridge_exit.dest_network);
    ...
    fn test_full_roundtrip_simulating_main() {
        ...
        let header = MultiBatchHeader::<Keccak256Hasher> {
            ...
            balances_proofs: vec![
                (
                    ...
                    (agglayer_primitives::U256::from(500), crate::local_balance_tree::LocalBalancePath::<Keccak256Hasher> {
                        siblings: [agglayer_primitives::Digest([0u8; 32]); crate::local_balance_tree::LOCAL_BALANCE_TREE_DEPTH],
                    })
                )
            ],
        ...
        let bridge_exits_zero_copy: Vec<BridgeExitZeroCopy> = 
            header.bridge_exits.iter().map(|be| BridgeExitZeroCopy::from_bridge_exit(be)).collect();
        let bridge_exits_bytes = bytemuck::cast_slice(&bridge_exits_zero_copy);
        
        let balances_proofs_zero_copy: Vec<TokenInfoZeroCopy> = 
            header.balances_proofs.iter().map(|(ti, _)| TokenInfoZeroCopy::from_token_info(ti)).collect();
        let balances_proofs_bytes = bytemuck::cast_slice(&balances_proofs_zero_copy);
        
        // Simulate main.rs reading and reconstructing data
        let mut aligned_buffer = [0u8; std::mem::size_of::<MultiBatchHeaderZeroCopy>()];
        aligned_buffer.copy_from_slice(&header_bytes);
        let header_zero_copy_read = bytemuck::from_bytes::<MultiBatchHeaderZeroCopy>(&aligned_buffer);
        
        let bridge_exits_read = bytemuck::cast_slice::<u8, BridgeExitZeroCopy>(&bridge_exits_bytes);
        let balances_proofs_read = bytemuck::cast_slice::<u8, TokenInfoZeroCopy>(&balances_proofs_bytes);
        ...
        let mut reconstructed_header = MultiBatchHeader::<Keccak256Hasher>::from_zero_copy(header_zero_copy_read).unwrap();
        
        // Convert zero-copy vectors back to original types
        reconstructed_header.bridge_exits = bridge_exits_read.into_iter().map(|be| be.to_bridge_exit()).collect();
        reconstructed_header.balances_proofs = balances_proofs_read.into_iter().map(|t| {
            let balance_path = crate::local_balance_tree::LocalBalancePath::<Keccak256Hasher> {
                siblings: [agglayer_primitives::Digest([0u8; 32]); crate::local_balance_tree::LOCAL_BALANCE_TREE_DEPTH],
            };
            (t.to_token_info(), (agglayer_primitives::U256::default(), balance_path))
        }).collect();
        ...
        assert_eq!(reconstructed_header.bridge_exits.len(), header.bridge_exits.len());
        assert_eq!(reconstructed_header.balances_proofs.len(), header.balances_proofs.len());
    }

    #[test]
    fn test_alignment_issue_reproduction() {
        // This test reproduces the exact alignment issue we see in main.rs
        // The problem is that sp1_zkvm::io::read_vec() returns data that might not be properly aligned
        ...
        // Try to read the unaligned data - this should fail with the same error we see in main.rs
        ...
        // Test that copying to an aligned buffer fixes the issue (like we tried in main.rs)
        let mut aligned_buffer = [0u8; std::mem::size_of::<MultiBatchHeaderZeroCopy>()];
        aligned_buffer.copy_from_slice(&header_bytes);
    ...
    fn test_zero_copy_roundtrip_performance() {
        ...
        println!("MultiBatchHeaderZeroCopy struct size: {} bytes", std::mem::size_of::<MultiBatchHeaderZeroCopy>());
        
        // Test that zero-copy roundtrip works correctly
        let start = std::time::Instant::now();
        let reconstructed = MultiBatchHeader::<Keccak256Hasher>::from_bytes_zero_copy(&zero_copy_bytes).unwrap();
