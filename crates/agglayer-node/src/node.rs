...
impl Node {
    ...
    pub(crate) async fn start(
        ...
        let mut rpc_service_handles = Vec::new();

        let readrpc_server = Self::serve_rpc(
            "ReadRPC",
            &mut rpc_service_handles,
            config.readrpc_addr(),
            config.readrpc_tls_addr(),
            readrpc_router,
            tls_config.as_ref(),
        );
        ...
        let rpc_handle = tokio::spawn(async move {
            ...
            for handle in rpc_service_handles {
                // Note: shutdown timeout is already applied on runtime level.
    ...
    fn serve_rpc(
        name: &str,
        handles: &mut Vec<axum_server::Handle>,
        ...
        let plain_server_fut = {
            ...
            let fut = axum_server::bind(addr_plain)
                .handle(handle.clone())
        ...
        let tls_server_fut = tls_config.map(|config| {
            let handle = axum_server::Handle::new();
            let fut = axum_server::bind_rustls(addr_tls, config.clone())
                .handle(handle.clone())
