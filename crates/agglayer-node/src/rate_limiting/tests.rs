...
fn per_network() {
    ...
    let rollup_ids = [2, 42, 1337, 95];

    for rollup_id in rollup_ids {
        assert_eq!(limiter.limit_send_tx(rollup_id, now), Ok(()));
    }

    for (rollup_id, offset) in rollup_ids.into_iter().zip([5, 9, 64, 99]) {
        let now = now + Duration::from_secs(offset);
        assert!(limiter.limit_send_tx(rollup_id, now).is_err());
    }

    for (rollup_id, offset) in rollup_ids.into_iter().zip([102, 105, 157, 455]) {
        let now = now + Duration::from_secs(offset);
        assert!(limiter.limit_send_tx(rollup_id, now).is_ok());
    }
}
