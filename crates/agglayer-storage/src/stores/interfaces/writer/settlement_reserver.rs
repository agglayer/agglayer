...
pub trait SettlementJobReserver: Send + Sync {
    ...
    /// `certificate_id` if one is already mapped; otherwise mints a fresh unique
    /// [`SettlementJobId`], verifies its uniqueness against the reverse index,
    /// and writes the forward and reverse mappings atomically.
