...
fn deserialize_custom_config_1() {
    ...
    assert_eq!(config.tx_retry_interval, Duration::from_secs(15));
    assert_eq!(config.confirmations, 64);
    assert_eq!(config.finality, Finality::Finalized);
    assert_eq!(config.gas_limit, U256::from(100_000_000_u64));
...
fn deserialize_custom_config_2() {
    ...
    assert_eq!(config.tx_retry_interval, Duration::from_secs(5));
    assert_eq!(config.confirmations, 16);
    assert_eq!(config.finality, Finality::Immediate);
    assert_eq!(config.gas_limit, U256::from(30_000_000_u64));
...
fn deserialize_full_settlement_config() {
    ...
    assert_eq!(
        config.validium_tx_config.tx_retry_interval,
        Duration::from_secs(5)
    );
    assert_eq!(config.validium_tx_config.confirmations, 16);
    assert_eq!(config.validium_tx_config.finality, Finality::Immediate);
    assert_eq!(
        config.validium_tx_config.gas_limit,
...
fn test_human_duration_formats() {
    let toml = r#"
        tx-retry-interval = "30s"
    "#;

    let config: SettlementTransactionConfig = toml::from_str(toml).unwrap();
    assert_eq!(config.tx_retry_interval, Duration::from_secs(30));
}
