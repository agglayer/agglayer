...
pub enum LiveTaskNotification {
    ...
    /// acting on stale in-memory state until its current retry sleep or
    /// deadline elapses, up to the non-inclusion backoff cap (10 minutes by
    /// default). Abort the task before editing when prompt observation
    /// matters.
    Queued,
