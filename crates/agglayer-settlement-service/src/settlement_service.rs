...
use crate::settlement_task::{
    derive_terminal_result_from_storage_records, SettlementTask, TaskAdminCommand,
};
    ...
    pub async fn retrieve_settlement_result(
        ...
        if self
            ...
            .unwrap_or_else(|error| {
                panic!(
                    "Failed to load settlement job {job_id} from storage while retrieving \
                     settlement result: {error}"
                )
            })
            .is_none()
        {
            panic!("No settlement job found for id {job_id} in memory or storage");
        }
        ...
        let attempts = self
            ...
            .unwrap_or_else(|error| {
                panic!(
                    "Failed to list settlement attempts for job {job_id} while retrieving \
                     settlement result: {error}"
                )
            });
        let attempt_results = self
            ...
            .unwrap_or_else(|error| {
                panic!(
                    "Failed to list settlement attempt results for job {job_id} while retrieving \
                     settlement result: {error}"
                )
            });

        if let Some(result) =
            derive_terminal_result_from_storage_records(&attempts, &attempt_results).unwrap_or_else(
                |error| {
                    panic!(
                        "Failed to derive terminal result from storage records for settlement job \
                         {job_id}: {error}"
                    )
                },
            )
        ...
        panic!("Settlement job {job_id} has no active watcher and no terminal result in storage")
    }
}
