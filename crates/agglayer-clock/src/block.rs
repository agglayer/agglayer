...
mod tests {
    ...
    async fn test_block_clock_with_genesis() {
        ...
        let clock =
            BlockClock::new_with_ws(ws, 2, NonZeroU64::new(3).unwrap(), Duration::from_secs(1))
                .await
                .expect("Failed to create BlockClock");
    ...
    async fn can_catchup_on_disconnection() {
        ...
        tokio::time::timeout(Duration::from_secs(1), async move {
            ...
            assert_eq!(recv.try_recv(), Ok(Event::EpochEnded(5)));
            assert_eq!(recv.try_recv(), Ok(Event::EpochEnded(6)));
        })
        .await
        .expect("Timeout");
        scenario.teardown();
