...
pub fn main() {
    ...
    let wrapped_proof0: AggregationProofOutput = AggregationProofOutput::bincode_options()
        .deserialize(&public_values0)
        .expect("Failed to deserialize");
    let wrapped_proof1: AggregationProofOutput = AggregationProofOutput::bincode_options()
        .deserialize(&public_values1)
        .expect("Failed to deserialize");

    let combined_proof_output = combine_proofs(wrapped_proof0, wrapped_proof1);

    let combined_proof_output = AggregationProofOutput::bincode_options()
        .serialize(&combined_proof_output)
        .unwrap();

    sp1_zkvm::io::commit_slice(&combined_proof_output);
}
