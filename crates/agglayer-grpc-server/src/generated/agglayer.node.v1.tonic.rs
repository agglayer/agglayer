...
pub mod node_state_service_server {
    ...
    pub trait NodeStateService: std::marker::Send + std::marker::Sync + 'static {
        ...
        async fn get_network_status(
            &self,
            request: tonic::Request<super::GetNetworkStatusRequest>,
        ) -> std::result::Result<
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                ...
                "/agglayer.node.v1.NodeStateService/GetNetworkStatus" => {
                    #[allow(non_camel_case_types)]
                    struct GetNetworkStatusSvc<T: NodeStateService>(pub Arc<T>);
                    impl<
                        T: NodeStateService,
                    > tonic::server::UnaryService<super::GetNetworkStatusRequest>
                    for GetNetworkStatusSvc<T> {
                        type Response = super::GetNetworkStatusResponse;
                        type Future = BoxFuture<
                            tonic::Response<Self::Response>,
                            tonic::Status,
                        >;
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as NodeStateService>::get_network_status(&inner, request)
                                    .await
                            };
                            Box::pin(fut)
                        }
